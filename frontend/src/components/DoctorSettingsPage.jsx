...
import DoctorIDCardModal from './DoctorIDCardModal';

/* ══════════════════════════════════════════
   SETTINGS TABS — Doctor version
══════════════════════════════════════════ */
const SETTINGS_TABS = [
...
/* ══════════════════════════════════════════
   ...
   Same navigation layout as Patient Settings
══════════════════════════════════════════ */
function DoctorSettingsPage({ user }) {
    ...
    const docId = user?.userId || user?.roleId || '—';
    ...
    const handleSaveProfile = async (e) => {
        ...
        try {
            await axiosInstance.put('/dashboard/doctor/profile', payload);
            updateUser(payload);
            setToast({ msg: 'Profile updated successfully.', type: 'success' });
        } catch (error) {
            setToast({ msg: error.response?.data?.message || 'Failed to save. Please try again.', type: 'error' });
        }
    };
    ...
    const handlePasswordSave = async (e) => {
        ...
        try {
            await axiosInstance.put('/dashboard/doctor/password', {
    ...
    const renderTabContent = () => {
            ...
            <form onSubmit={handleSaveProfile}>
                ...
                <div className="pd-settings-body">
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 20, marginBottom: 24, paddingBottom: 20, borderBottom: '1px solid #E5E7EB' }}>
                        <div style={{ width: 80, height: 80, borderRadius: 16, background: '#EFF6FF', border: '2px solid #DBEAFE', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', fontSize: 30, color: '#1A47DB', flexShrink: 0 }}>
                            {form.profilePhoto ?
                    ...
                    {/* Row 1: Full Name / Speciality */}
                    <div className="pd-form-row">
                        <div className="pd-form-group">
                            <label><i className="bi bi-person" style={{ marginRight: 6, color: '#1A47DB' }} />Full Name</label>
                            <input className="pd-form-input" name="fullName" value={form.fullName} onChange={handleChange} placeholder="Dr. Sarah Johnson" />
                        </div>
                        <div className="pd-form-group">
                            <label><i className="bi bi-shield-plus" style={{ marginRight: 6, color: '#1A47DB' }} />Specialty</label>
                            <input className="pd-form-input" name="specialization" value={form.specialization} onChange={handleChange} placeholder="Internal Medicine" />
                        </div>
                    </div>
                    ...
                    <div className="pd-form-row">
                        <div className="pd-form-group">
                            <label><i className="bi bi-building" style={{ marginRight: 6, color: '#1A47DB' }} />Department</label>
                            <input className="pd-form-input" name="department" value={form.department} onChange={handleChange} placeholder="General Medicine" />
                        </div>
                        <div className="pd-form-group">
                            <label><i className="bi bi-clock-history" style={{ marginRight: 6, color: '#1A47DB' }} />Years of Experience</label>
                            <input className="pd-form-input" name="experience" type="number" value={form.experience} onChange={handleChange} placeholder="12" />
                        </div>
                    </div>
                    ...
                    <div className="pd-form-row">
                        <div className="pd-form-group">
                            <label><i className="bi bi-telephone" style={{ marginRight: 6, color: '#1A47DB' }} />Phone Number</label>
                            <input className="pd-form-input" name="phone" value={form.phone} onChange={handleChange} placeholder="+1 (555) 234-5678" />
                        </div>
                        <div className="pd-form-group">
                            <label><i className="bi bi-envelope" style={{ marginRight: 6, color: '#1A47DB' }} />Email Address</label>
                            <input className="pd-form-input" type="email" name="email" value={form.email} onChange={handleChange} placeholder="doctor@healthlinkhub.com" />
                        </div>
                    </div>

                    {/* Row 4: License / Hospital */}
                    <div className="pd-form-row">
                        <div className="pd-form-group">
                            <label><i className="bi bi-patch-check" style={{ marginRight: 6, color: '#1A47DB' }} />Medical License Number</label>
                            <input className="pd-form-input" name="medicalLicenseNumber" value={form.medicalLicenseNumber} onChange={handleChange} placeholder="MED-LIC-9821-TX" />
                        </div>
                        <div className="pd-form-group">
                            <label><i className="bi bi-hospital" style={{ marginRight: 6, color: '#1A47DB' }} />Hospital / Clinic Name</label>
                            <input className="pd-form-input" name="hospitalName" value={form.hospitalName} onChange={handleChange} placeholder="HealthLink Hospital" />
                        </div>
                    </div>

                    {/* Row 5: Address (full width) */}
                    <div className="pd-form-full pd-form-group">
                        <label><i className="bi bi-geo-alt" style={{ marginRight: 6, color: '#1A47DB' }} />Address</label>
                        <input className="pd-form-input" name="address" value={form.address} onChange={handleChange} placeholder="123 Medical Center Blvd, Suite 200" />
                    </div>
        ...
        if (activeTab === 'idcard') return (
            <div>
                <div className="pd-settings-content-head">
                    <div className="pd-settings-content-title">Doctor Digital ID Card</div>
                </div>
                <div className="pd-settings-body">
                    {/* Preview Card */}
                    <div
                        style={{ background: 'linear-gradient(130deg, #1A47DB 0%, #2563EB 40%, #0EA5E9 100%)', borderRadius: 16, padding: '24px', color: '#fff', display: 'flex', alignItems: 'center', gap: 20, cursor: 'pointer', transition: 'transform 0.2s, box-shadow 0.2s', marginBottom: 18 }}
                        onClick={() => setShowIdCard(true)}
                        onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 8px 32px rgba(26,71,219,0.25)'; }}
                    ...
                    >
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ fontWeight: 800, fontSize: 17 }}>Dr. {user?.fullName || 'Doctor'}</div>
                            <div style={{ fontSize: 12, opacity: 0.85, marginBottom: 6 }}>{docId}</div>
                            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                                {user?.specialization && <span style={{ background: 'rgba(255,255,255,0.18)', border: '1px solid rgba(255,255,255,0.28)', borderRadius: 20, padding: '3px 10px', fontSize: 11, fontWeight: 500 }}>{user.specialization}</span>}
                                {user?.hospitalName && <span style={{ background: 'rgba(255,255,255,0.18)', border: '1px solid rgba(255,255,255,0.28)', borderRadius: 20, padding: '3px 10px', fontSize: 11, fontWeight: 500 }}>🏥 {user.hospitalName}</span>}
                            </div>
                        </div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
                        <div>
                            ...
                            <div style={{ fontSize: 13, color: '#6B7280' }}>Click the card above or the button to view, download, or print your doctor ID.</div>
                        </div>
                        <button type="button" onClick={() => setShowIdCard(true)} className="pd-save-btn" style={{ background: '#16A34A' }}>
    ...
    /* ══════════════════════════════════════════
       RENDER — Same layout as Patient Settings
    ══════════════════════════════════════════ */
    return (
        <>
            <div className="pd-settings-wrap">
                {/* Left panel — Nav + Doctor Info Card */}
                <div className="pd-settings-panel">
                    <nav className="pd-settings-nav mb-4 mb-md-0">
                        ...
                        {/* ID Card tab — visible as nav item */}
                        <button
                            type="button"
                    ...
                    >
                        ...
                        <div className="pd-pic-name">Dr. {user?.fullName || 'Doctor'}</div>
                        <div className="pd-pic-id">{docId}</div>
                        <div className="pd-pic-meta">
                            {user?.specialization && (
                                <div className="pd-pic-meta-row">
                                    <i className="bi bi-shield-plus" />Specialty: {user.specialization}
                                </div>
                            )}
                            {user?.department && (
                                <div className="pd-pic-meta-row">
                                    <i className="bi bi-building" />Dept: {user.department}
                                </div>
                            )}
                            {user?.hospitalName && (
                                <div className="pd-pic-meta-row">
                                    <i className="bi bi-hospital" />🏥 {user.hospitalName}
                                </div>
                            )}
                        </div>
            ...
            {/* ID CARD MODAL */}
            {showIdCard && <DoctorIDCardModal user={user} onClose={() => setShowIdCard(false)} />}
        </>
    );
}
