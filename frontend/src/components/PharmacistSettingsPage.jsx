...
import PharmacistIDCardModal from './PharmacistIDCardModal';

/* ══════════════════════════════════════════
   SETTINGS TABS — Pharmacist version
══════════════════════════════════════════ */
const SETTINGS_TABS = [
...
/* ══════════════════════════════════════════
   PHARMACIST SETTINGS PAGE
══════════════════════════════════════════ */
function PharmacistSettingsPage({ user }) {
    ...
    const pharmacistId = user?.userId || user?.roleId || '—';
    ...
    const handleSaveProfile = async (e) => {
        ...
        try {
            await axiosInstance.put('/dashboard/pharmacy/profile', payload);
            updateUser(payload);
            setToast({ msg: 'Profile updated successfully.', type: 'success' });
        } catch (error) {
    ...
    const handlePasswordSave = async (e) => {
        ...
        try {
            await axiosInstance.put('/dashboard/pharmacy/password', {
    ...
    const renderTabContent = () => {
            ...
            <form onSubmit={handleSaveProfile}>
                ...
                <div className="pd-settings-body">
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 20, marginBottom: 24, paddingBottom: 20, borderBottom: '1px solid #E5E7EB' }}>
                        <div style={{ width: 80, height: 80, borderRadius: 16, background: '#F0FDF4', border: '2px solid #BBF7D0', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', fontSize: 30, color: '#16A34A', flexShrink: 0 }}>
                            {form.profilePhoto ?
                    ...
                    {/* Row 1: Full Name / Pharmacy Name */}
                    <div className="pd-form-row">
                        <div className="pd-form-group">
                            <label><i className="bi bi-person" style={{ marginRight: 6, color: '#16A34A' }} />Full Name</label>
                            <input className="pd-form-input" name="fullName" value={form.fullName} onChange={handleChange} placeholder="John Doe" />
                        </div>
                        <div className="pd-form-group">
                            <label><i className="bi bi-building" style={{ marginRight: 6, color: '#16A34A' }} />Pharmacy Name</label>
                            <input className="pd-form-input" name="pharmacyName" value={form.pharmacyName} onChange={handleChange} placeholder="Central Pharmacy" />
                        </div>
                    </div>
                    ...
                    <div className="pd-form-row">
                        <div className="pd-form-group">
                            <label><i className="bi bi-telephone" style={{ marginRight: 6, color: '#16A34A' }} />Phone Number</label>
                            <input className="pd-form-input" name="phone" value={form.phone} onChange={handleChange} placeholder="+1 (555) 234-5678" />
                        </div>
                        <div className="pd-form-group">
                            <label><i className="bi bi-envelope" style={{ marginRight: 6, color: '#16A34A' }} />Email Address</label>
                            <input className="pd-form-input" type="email" name="email" value={form.email} onChange={handleChange} placeholder="pharmacy@healthlinkhub.com" />
                        </div>
                    </div>

                    {/* Row 3: Address (full width) */}
                    <div className="pd-form-full pd-form-group">
                        <label><i className="bi bi-geo-alt" style={{ marginRight: 6, color: '#16A34A' }} />Address</label>
                        <input className="pd-form-input" name="address" value={form.address} onChange={handleChange} placeholder="123 Medical Center Blvd, Pharmacy Level 1" />
                    </div>

                    {/* Footer */}
                    <div className="pd-form-footer">
                        <span className="pd-form-footer-note">Changes will apply immediately after saving.</span>
                        <button type="submit" className="pd-save-btn" style={{ background: '#16A34A' }}>
        ...
        if (activeTab === 'idcard') return (
            <div>
                <div className="pd-settings-content-head">
                    <div className="pd-settings-content-title">Pharmacist Digital ID Card</div>
                </div>
                <div className="pd-settings-body">
                    {/* Preview Card */}
                    <div
                        style={{ background: 'linear-gradient(130deg, #14B8A6 0%, #10B981 100%)', borderRadius: 16, padding: '24px', color: '#fff', display: 'flex', alignItems: 'center', gap: 20, cursor: 'pointer', transition: 'transform 0.2s, box-shadow 0.2s', marginBottom: 18 }}
                        onClick={() => setShowIdCard(true)}
                        onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 8px 32px rgba(20,184,166,0.25)'; }}
                    ...
                    >
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ fontWeight: 800, fontSize: 17 }}>{user?.fullName || 'Pharmacist'}</div>
                            <div style={{ fontSize: 12, opacity: 0.85, marginBottom: 6 }}>{pharmacistId}</div>
                            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                                <span style={{ background: 'rgba(255,255,255,0.18)', border: '1px solid rgba(255,255,255,0.28)', borderRadius: 20, padding: '3px 10px', fontSize: 11, fontWeight: 500 }}>Pharmacist</span>
                                {user?.pharmacyName && <span style={{ background: 'rgba(255,255,255,0.18)', border: '1px solid rgba(255,255,255,0.28)', borderRadius: 20, padding: '3px 10px', fontSize: 11, fontWeight: 500 }}>🏥 {user.pharmacyName}</span>}
                            </div>
                        </div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
                        <div>
                            ...
                            <div style={{ fontSize: 13, color: '#6B7280' }}>Click the card above or the button to view, download, or print your Pharmacist ID.</div>
                        </div>
    ...
    return (
        <>
            <div className="pd-settings-wrap">
                ...
                <div className="pd-settings-panel">
                    ...
                    >
                        ...
                        <div className="pd-pic-name">{user?.fullName || 'Pharmacist'}</div>
                        <div className="pd-pic-id">{pharmacistId}</div>
                        <div className="pd-pic-meta">
                            <div className="pd-pic-meta-row">
                                <i className="bi bi-building" />Pharmacy: {user?.pharmacyName || 'HealthLink Pharmacy'}
                            </div>
                        </div>
            ...
            {showIdCard && <PharmacistIDCardModal user={user} onClose={() => setShowIdCard(false)} />}
        </>
    );
}
