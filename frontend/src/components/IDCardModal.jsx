...
const IDCardModal = ({ user, onClose }) => {
    ...
    const downloadPNG = async () => {
        ...
        link.download = `HealthLink_Patient_ID_${cleanId}.png`;
    ...
    const downloadPDF = async () => {
        ...
        const pdfWidth = 85.6; // CR80 standard length
        
        // Calculate height proportional to the captured canvas
        const ratio = canvas.height / canvas.width;
        const pdfHeight = pdfWidth * ratio; 
        
        pdf.text("HealthLink Hub - Patient ID Card", 10, 15);
        pdf.addImage(imgData, 'PNG', 10, 25, pdfWidth, pdfHeight);
        pdf.save(`HealthLink_Patient_ID_${cleanId}.pdf`);
    };
    ...
    const FrontCard = () => (
        <div style={{ width: '340px', height: '215px', background: '#ffffff', borderRadius: '12px', overflow: 'hidden', border: `2px solid ${type.color}`, position: 'relative', boxSizing: 'border-box', fontFamily: 'sans-serif', margin: '0 auto' }}>
            ...
            <div style={{ padding: '10px', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: 'calc(100% - 44px)' }}>
                <div style={{ width: '64px', height: '64px', borderRadius: '50%', border: `3px solid ${type.color}`, marginBottom: '8px', overflow: 'hidden', background: '#f8fafc', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '28px', color: type.color, flexShrink: 0 }}>
                ...
                <div style={{ fontWeight: 800, fontSize: '16px', color: '#1f2937', marginBottom: '2px', width: '100%', padding: '0 10px', wordBreak: 'break-word', textAlign: 'center', lineHeight: '1.2', flexShrink: 0 }}>
                    {type.namePrefix}{user.fullName || 'Patient'}
                </div>
                <div style={{ fontSize: '12px', color: type.color, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '2px' }}>
                    {type.roleLabel}
                </div>
                <div style={{ marginTop: 'auto', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
                    <div style={{ fontSize: '13px', fontWeight: 700, color: '#111827', background: '#f3f4f6', padding: '5px 16px', borderRadius: '12px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                        <i className="bi bi-person-vcard"></i> {type.idFormat}
                    </div>
                    <div style={{ fontSize: '11.5px', fontWeight: 700, color: '#DC2626', display: 'flex', alignItems: 'center', gap: '4px' }}>
                        <i className="bi bi-droplet-fill"></i> Blood Group: {user.bloodGroup || '—'}
                    </div>
                </div>
            </div>
        </div>
        ...
        <div style={{ width: '340px', height: '215px', background: '#ffffff', borderRadius: '12px', overflow: 'hidden', border: `2px solid ${type.color}`, position: 'relative', boxSizing: 'border-box', fontFamily: 'sans-serif', margin: '0 auto' }}>
            ...
            <div style={{ padding: '12px 16px', display: 'flex', alignItems: 'center', height: 'calc(100% - 32px)', gap: '16px' }}>
                ...
                <div style={{ flex: 1, fontSize: '11px', color: '#374151', lineHeight: '1.5', textAlign: 'left' }}>
                    <div style={{ fontWeight: 800, color: type.color, fontSize: '13px', marginBottom: '4px' }}>Patient Contact</div>
                    ...
                    <div style={{ fontWeight: 500, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>Address: {user.address || '—'}</div>
                    <div style={{ marginTop: '10px', fontSize: '9.5px', color: '#6b7280', borderTop: '1px solid #e5e7eb', paddingTop: '6px', lineHeight: '1.3' }}>
                        HealthLink Hub Hospital<br/>
                        For internal medical access & patient ID scanning.
                    </div>
                </div>
            </div>
        </div>
    ...
    return (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 1050, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }} onClick={onClose}>
            ...
            <div className="modal-override" style={{ background: '#f8fafc', borderRadius: '16px', width: '100%', maxWidth: '500px', padding: '24px', position: 'relative', boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.1), 0 10px 10px -5px rgba(0, 0, 0, 0.04)' }} onClick={e => e.stopPropagation()}>
                ...
                <h4 className="no-print" style={{ textAlign: 'center', color: '#0f172a', fontWeight: '800', marginBottom: '6px' }}>Patient ID Card</h4>
                ...
                {/* HIDDEN RENDER FOR EXPORT AND PRINT (Unflipped side-by-side or stacked format) */}
