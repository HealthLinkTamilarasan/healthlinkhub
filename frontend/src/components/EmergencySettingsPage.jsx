...
import EmergencyIDCardModal from './EmergencyIDCardModal';
...
function EmergencySettingsPage({ user }) {
    ...
    const emgId = user?.roleId || user?.userId || '—';
    ...
    const handleSaveProfile = async (e) => {
        ...
        try {
            await axiosInstance.put('/emergency/profile', payload);
            updateUser(payload);
            setToast({ msg: 'Profile updated successfully.', type: 'success' });
        } catch (error) {
            setToast({ msg: error.response?.data?.message || 'Failed to save.', type: 'error' });
        }
    };
    ...
    const handlePasswordSave = async (e) => {
        ...
        try {
            await axiosInstance.put('/emergency/password', {
    ...
    const renderTabContent = () => {
            ...
            <form onSubmit={handleSaveProfile}>
                ...
                <div className="pd-settings-body">
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 20, marginBottom: 24, paddingBottom: 20, borderBottom: '1px solid #E5E7EB' }}>
                        <div style={{ width: 80, height: 80, borderRadius: 16, background: '#FEF2F2', border: '2px solid #FECACA', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', fontSize: 30, color: '#DC2626', flexShrink: 0 }}>
                            {form.profilePhoto ?
                        ...
                        <div>
                            ...
                            <div style={{ fontSize: 13, color: '#6B7280', marginBottom: 12 }}>Upload a team photo (JPG, PNG).</div>
                    ...
                    <div className="pd-form-row">
                        <div className="pd-form-group">
                            <label><i className="bi bi-person" style={{ marginRight: 6, color: '#DC2626' }} />Full Name</label>
                            <input className="pd-form-input" name="fullName" value={form.fullName} onChange={handleChange} placeholder="John Smith" />
                        </div>
                        <div className="pd-form-group">
                            <label><i className="bi bi-people-fill" style={{ marginRight: 6, color: '#DC2626' }} />Team Name</label>
                            <input className="pd-form-input" name="teamName" value={form.teamName} onChange={handleChange} placeholder="Alpha Response Unit" />
                        </div>
                    </div>

                    {/* Row 2: Team Number / Phone */}
                    <div className="pd-form-row">
                        <div className="pd-form-group">
                            <label><i className="bi bi-hash" style={{ marginRight: 6, color: '#DC2626' }} />Team Number</label>
                            <input className="pd-form-input" name="teamNumber" value={form.teamNumber} onChange={handleChange} placeholder="T-001" />
                        </div>
                        <div className="pd-form-group">
                            <label><i className="bi bi-telephone" style={{ marginRight: 6, color: '#DC2626' }} />Phone Number</label>
                    ...
                    <div className="pd-form-row">
                        <div className="pd-form-group">
                            <label><i className="bi bi-envelope" style={{ marginRight: 6, color: '#DC2626' }} />Email Address</label>
                            <input className="pd-form-input" type="email" name="email" value={form.email} onChange={handleChange} placeholder="team@healthlinkhub.com" />
                        </div>
                        <div className="pd-form-group">
                            <label><i className="bi bi-geo-alt" style={{ marginRight: 6, color: '#DC2626' }} />Address</label>
                            <input className="pd-form-input" name="address" value={form.address} onChange={handleChange} placeholder="123 Emergency Lane" />
                        </div>
                    </div>

                    <div className="pd-form-footer">
                        <span className="pd-form-footer-note">Changes will apply immediately after saving.</span>
                        <button type="submit" className="pd-save-btn" style={{ background: '#DC2626' }}>
            ...
            <form onSubmit={handlePasswordSave}>
                ...
                <div className="pd-settings-body">
                    ...
                    <div className="pd-form-footer">
                        ...
                        <button type="submit" className="pd-save-btn" style={{ background: '#9333EA' }}>
        ...
        if (activeTab === 'idcard') return (
            <div>
                <div className="pd-settings-content-head">
                    <div className="pd-settings-content-title">Emergency Team Digital ID Card</div>
                </div>
                <div className="pd-settings-body">
                    <div
                        style={{ background: 'linear-gradient(130deg, #DC2626 0%, #EF4444 40%, #F87171 100%)', borderRadius: 16, padding: '24px', color: '#fff', display: 'flex', alignItems: 'center', gap: 20, cursor: 'pointer', transition: 'transform 0.2s, box-shadow 0.2s', marginBottom: 18 }}
                        onClick={() => setShowIdCard(true)}
                        onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 8px 32px rgba(220,38,38,0.25)'; }}
                    ...
                    >
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ fontWeight: 800, fontSize: 17 }}>{user?.fullName || 'Team Member'}</div>
                            <div style={{ fontSize: 12, opacity: 0.85, marginBottom: 6 }}>{emgId}</div>
                            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                                {user?.teamName && <span style={{ background: 'rgba(255,255,255,0.18)', border: '1px solid rgba(255,255,255,0.28)', borderRadius: 20, padding: '3px 10px', fontSize: 11, fontWeight: 500 }}>{user.teamName}</span>}
                                {user?.teamNumber && <span style={{ background: 'rgba(255,255,255,0.18)', border: '1px solid rgba(255,255,255,0.28)', borderRadius: 20, padding: '3px 10px', fontSize: 11, fontWeight: 500 }}>#{user.teamNumber}</span>}
                            </div>
                        </div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
                        <div>
                            ...
                            <div style={{ fontSize: 13, color: '#6B7280' }}>Click the card above or the button to view, download, or print your emergency team ID.</div>
                        </div>
                        <button type="button" onClick={() => setShowIdCard(true)} className="pd-save-btn" style={{ background: '#DC2626' }}>
        ...
        <>
            <div className="pd-settings-wrap">
                <div className="pd-settings-panel">
                    ...
                    {/* Emergency Info Card (PC ONLY) */}
                    <div
                        className="pd-patient-info-card d-none d-md-block"
                        onClick={() => setShowIdCard(true)}
                        style={{ cursor: 'pointer', transition: 'transform 0.2s', marginTop: 24, background: 'linear-gradient(135deg, #DC2626 0%, #F87171 100%)' }}
                    ...
                    >
                        ...
                        <div className="pd-pic-name">{user?.fullName || 'Team Member'}</div>
                        <div className="pd-pic-id">{emgId}</div>
                        <div className="pd-pic-meta">
                            {user?.teamName && (
                                <div className="pd-pic-meta-row">
                                    <i className="bi bi-people-fill" />Team: {user.teamName}
                                </div>
                            )}
                            {user?.teamNumber && (
                                <div className="pd-pic-meta-row">
                                    <i className="bi bi-hash" />Number: {user.teamNumber}
                                </div>
                            )}
                            {user?.emergencyRole && (
                                <div className="pd-pic-meta-row">
                                    <i className="bi bi-shield-fill" />Role: {user.emergencyRole}
                                </div>
                            )}
                        </div>
            ...
            {showIdCard && <EmergencyIDCardModal user={user} onClose={() => setShowIdCard(false)} />}
        </>
    );
}
